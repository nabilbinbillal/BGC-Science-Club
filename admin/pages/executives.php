...
// ensure upload helper is available
...
$columnsToCheck = [
    ...
    // legacy DBs used 'additional_links' — make sure we expose a 'social_links' column used by the app
    'social_links' => "ALTER TABLE executives ADD COLUMN social_links TEXT DEFAULT NULL",
...
foreach ($columnsToCheck as $column => $alterQuery) {
    ...
    if ($check->rowCount() == 0) {
        $pdo->exec($alterQuery);
        // If the DB has legacy 'additional_links' column, copy its content into social_links
        if ($column === 'social_links') {
            $hasAdditional = $pdo->query("SHOW COLUMNS FROM executives LIKE 'additional_links'")->rowCount() > 0;
            if ($hasAdditional) {
                // Copy older values over when social_links is empty
                $pdo->exec("UPDATE executives SET social_links = additional_links WHERE (social_links IS NULL OR social_links = '') AND (additional_links IS NOT NULL AND additional_links != '')");
            }
        }
    ...
    try {
        if (isset($_POST['action'])) {
            ...
            if ($action == 'add' || $action == 'edit') {
                ...
                // keep a value for the DB 'role' column too (many schemas expect role not position)
                $roleValue = $position;
                ...
                if (!empty($_FILES['profile_pic']['name']) && $_FILES['profile_pic']['error'] === UPLOAD_ERR_OK) {
                    ...
                    $target_file = $target_dir . $profilePic;
                        // Use central upload helper to compress and convert to webp when appropriate
                        $uploadResult = uploadMemberImage($_FILES['profile_pic'], $target_dir);
                        if (!$uploadResult) {
                ...
                if ($action == 'add') {
                    ...
                    $archive_year = null;
                    $stmt = $pdo->prepare("INSERT INTO executives (name, type, department, position, role, bio, email, phone, facebook, twitter, linkedin, instagram, profile_pic, slug, sort_order, archive_year, website, roll_no, social_links) 
                                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                    $stmt->execute([$name, $type, $department, $position, $roleValue, $bio, $email, $phone, $facebook, $twitter, $linkedin, $instagram, $profilePic, $slug, $sortOrder, $archive_year, $website, $roll_no, $social_links]);
                    $_SESSION['success'] = "Executive added successfully!";
                } else {
                    $id = $_POST['id'];
                    $updateQuery = "UPDATE executives SET name = ?, type = ?, department = ?, position = ?, role = ?, bio = ?, email = ?, phone = ?, facebook = ?, twitter = ?, linkedin = ?, instagram = ?, slug = ?, website = ?, roll_no = ?, social_links = ?";
                    $params = [$name, $type, $department, $position, $roleValue, $bio, $email, $phone, $facebook, $twitter, $linkedin, $instagram, $slug, $website, $roll_no, $social_links];
        ...
        // Log the error for easier debugging and expose a friendly message to the UI
        error_log('Admin Executives error: ' . $e->getMessage());
